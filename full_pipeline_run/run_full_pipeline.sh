...
cd "$ROOT"
rm -f "$LOG_DIR/stage_status.tsv"
printf 'stage\tdescription\tlog\n' >"$LOG_DIR/stage_status.tsv"

{
  printf 'Full Dudleya organelle pipeline rerun\n'
  printf 'Started: %s\n' "$(date -Is)"
  printf 'Root: %s\n' "$ROOT"
  printf 'Run directory: %s\n' "$RUN_DIR"
  printf 'Threads: %s\n' "$THREADS"
  printf 'Tool PATH prefix: %s\n' "$ROOT/.tools/bioconda-env/bin"
} >"$RUN_DIR/run_metadata.txt"

run_stage 00_manifest "Sample manifest and preflight validation" \
  "$PYTHON" dudleya_organelle_alignment_pipeline/scripts/build_sample_manifest.py \
  --output-dir "$RESULTS/00_manifest"

PATH="$TOOL_PATH" run_stage 01_reference_pilot "Reference validation, indexing, and pilot sample selection" \
  "$PYTHON" dudleya_organelle_alignment_pipeline/scripts/prepare_reference_and_pilot.py \
  --analysis-samples "$RESULTS/00_manifest/analysis_samples.tsv" \
  --output-dir "$RESULTS/01_reference_pilot"

PATH="$TOOL_PATH" run_stage 02_pilot_alignment "Pilot organelle alignment" \
  "$PYTHON" dudleya_organelle_alignment_pipeline/scripts/run_pilot_alignment.py \
  --pilot-table "$RESULTS/01_reference_pilot/pilot_samples.tsv" \
  --output-dir "$RESULTS/02_pilot_alignment" \
  --threads "$THREADS" \
  --force

run_stage 03_mtdna_investigation "Archive mtDNA investigation evidence consumed by Stage 05" \
  cp -a dudleya_organelle_alignment_pipeline/results/03_mtdna_investigation "$RESULTS/03_mtdna_investigation"

run_stage 04_cpdna_investigation "Archive cpDNA investigation evidence consumed by Stage 05" \
  cp -a dudleya_organelle_alignment_pipeline/results/04_cpdna_investigation "$RESULTS/04_cpdna_investigation"

run_stage 05_analysis_masks "Analysis masks and population tracks" \
  "$PYTHON" dudleya_organelle_alignment_pipeline/scripts/build_analysis_masks.py \
  --output-dir "$RESULTS/05_analysis_masks" \
  --cpdna-repeat-intervals "$RESULTS/04_cpdna_investigation/cpdna_self_repeat_intervals.tsv" \
  --mtdna-high-mapq-intervals "$RESULTS/03_mtdna_investigation/mtdna_high_mapq_consensus_intervals.tsv"

PATH="$TOOL_PATH" run_stage 06_all_sample_alignment "All-sample organelle alignment and track-aware QC" \
  "$PYTHON" dudleya_organelle_alignment_pipeline/scripts/run_all_sample_alignment.py \
  --sample-table "$RESULTS/00_manifest/analysis_samples.tsv" \
  --track-table "$RESULTS/05_analysis_masks/analysis_tracks.tsv" \
  --output-dir "$RESULTS/06_all_sample_alignment" \
  --threads "$THREADS" \
  --force
