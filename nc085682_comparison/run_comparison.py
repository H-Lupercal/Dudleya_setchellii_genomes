...
CANDIDATE_RAW = ROOT / "Dudleya_hifiasm_purged_manual_chloroplast.fa"
CACHED_GB = ROOT / "gateVER" / "references" / "chloroplast_references.gb"
DRAFT_TSV = ROOT / "gateVER" / "chloroplast.best_nonredundant.annotation.tsv"
EXISTING_SUMMARY = ROOT / "gateVER" / "chloroplast_vs_chloroplast_reference_genomes.summary.tsv"
...
def fetch_current_records(allow_cached_fallback: bool) -> tuple[str | None, str | None, str]:
    ...
    try:
        ...
        return None, None, f"cached gateVER GenBank fallback; live fetch failed: {exc}"
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Use the cached gateVER GenBank record only if the live NCBI download fails. "
